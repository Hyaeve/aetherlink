...
// GUA）—— 按类型它是公网，可它就在 AetherLink 所在的局域网里。设置页的
// 「内网网段」能补上，但运营商的前缀会变（重新拨号 / 光猫重启），填死迟早过期。
//
// 于是再加一层不需要维护的判据：**凡是与 AetherLink 本机处于同一网段的客户端，
// 一律按内网处理**。前缀随运营商变时，本机地址跟着一起变，这一层自动跟随。
// 前提是容器能看到局域网网段（host / macvlan 网络；bridge 网络里只有 172.x）。
//
// 这一层是推算出来的、用户看不见，所以它必须出声：命中的判定会带上一句
// 「与本机在同一网段 …」，见 ScopeOfClientWithReason 与设置页那行提示。
