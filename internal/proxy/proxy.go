...
func (s *Server) serveMedia(writer http.ResponseWriter, request *http.Request, ref upstream.MediaRef) {
	...
	if intranetTargetPublicClient {
		// 直链是内网地址而客户端在外网时，302 出去也连不上，只能中继。
		wantRedirect = false
	}

	if wantRedirect {
		event.StatusCode = http.StatusFound
		// 302 这一行是它唯一的输出口，附加说明必须写进去——不然「跟着上游跳了
		// 几跳」「直链是内网地址」这几件事用户看不到。
	...
	if intranetTargetPublicClient {
		finish(stats.OutcomeProxyStream, cacheNote(event)+"；直链是内网地址而客户端在外网，本次由 AetherLink 中继")
		return
	}
