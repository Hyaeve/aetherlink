...
**所有配置都在网页上完成。** compose 只需要一个 `./config` 卷：管理口令、上游地址与 API 密钥、302 策略、路径映射全部在管理界面填写，保存即生效、无需重启容器。

## 工作原理

```
播放器 ──▶ AetherLink ──▶ (非媒体请求) ──▶ Audiobookshelf / Emby
                │
...
浏览器打开 `http://宿主机:15151/aetherlink/`：

1. 首次访问会出现初始化向导，设置一个管理口令（至少 8 位）。口令只以 PBKDF2-SHA256 的派生值存进 `config/config.yaml`，明文不落盘。
...
| 页面 | 作用 |
| --- | --- |
| 概览 | 302 / 中继 / 本地直读 / 透传 / 失败计数，缓存命中率，最近媒体请求明细（含指针类型与耗时）。 |
| 反代上游 | 每个上游一张卡片，卡片直接显示名称、服务端类型、`地址:端口`与状态（缺密钥 / 已停用 / 未挂载）。左键卡片打开详细编辑窗口，右键弹出菜单（详细编辑 / 测试连接 / 停用启用 / 删除）。保存立即生效。 |
| 书库浏览 | 按媒体库分页浏览条目，展开查看每个文件的 `.strm` 解析结果，单文件「验证 302」并给出最终直链。 |
| STRM 实验室 | 粘贴任意指针内容，检查归一化结果、指针类型与显示文件名；可套用某上游的路径规则。 |
| 日志 | 内存环形缓冲的运行日志，按级别过滤。 |
| 设置 | 302 跳转策略、缓存、日志级别、修改管理口令。 |

界面是莫兰迪低饱和浅色主题，左侧一列图标做页面切换，鼠标悬停可看页面名。
...
| 反代入口前缀 | 该上游在 AetherLink 上的挂载路径，同时反代多个服务时用它区分（单个上游填 `/`）。 |
...
| `AETHERLINK_ADMIN_TOKEN` | **应急令牌**，可绕过口令登录。仅在忘记管理口令时临时加上，排障后请移除。 |
| `AETHERLINK_LISTEN`、`AETHERLINK_LOG_LEVEL`、`AETHERLINK_REDIRECT_MODE`、`AETHERLINK_FOLLOW_REDIRECTS` | 启动期覆盖，用于排障。 |

忘记口令的另一种恢复方式：删掉 `config/config.yaml` 里的整个 `auth:` 段并重启容器，界面会重新回到初始化向导，上游配置不受影响。

## 安全说明

- 管理界面有独立口令，与上游账号无关。未登录访问管理 API 返回 401，未初始化时返回 403 并提示走向导；只有 `/aetherlink/api/health` 与初始化状态查询免鉴权。
- 会话令牌只存在内存里，容器重启后全部失效；修改口令会立即注销所有会话。
- 配置接口只回报 `hasApiKey` 布尔值，不回显上游密钥，也不回显任何口令材料。`config.yaml` 以 0600 权限原子写入。
...
- 监听地址无法热改：在配置里改了 `server.listen` 之后界面会提示需要重启容器。对外端口请改 compose 的 `ports`。
