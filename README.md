...
| 飞牛影视偶尔只有一两条播放不跳转 | PlaybackInfo 改写后的媒体源只缓存 10 分钟，之后要么由客户端重新协商、要么由 AetherLink 自己回头查一次上游。没填账号密码时这一查必然失败，那一条就会退化成透传（其余播放不受影响）。想让任何情况下都能补上，就在该上游里填飞牛的登录账号与密码。 |
| 能进库、能浏览，一播放就失败：`解析 strm 目标失败 ... invalid character '<' looking for beginning of value` | 上游把 AetherLink「取条目」的请求当成了不认识的路径，回了一整页网页（还带着 HTTP 200）而不是 JSON，解码在第一个 `<` 上就断了。根因是飞牛影视只实现了单项路由 `/Items/{id}`，没有集合路由 `/Items?Ids=`。现在会按上游类型自动选路（飞牛单项优先、Emby 集合优先），并且失败时把「哪条路由、上游返回的 `Content-Type` 与响应开头」一并写进错误信息，可以直接照着定位。升级到修复版本后重启容器即可。 |
...
| 查看已保存的密钥 / 密码 | 秘密默认不回显，编辑窗口里对应字段的标题右侧有一个「显示」按钮，点一下才向服务端取回原值并填进输入框，服务端会记一行审计日志；再点「收起」清空字段、恢复「留空即保留原值」。新增上游时没有旧值可取，按钮不会出现。这样钥匙不摆在屏幕上，避免随截图或录屏外流。 |
...
- 配置接口只回报 `hasApiKey` 布尔值，不回显上游密钥；飞牛的登录账号会回显（它不是秘密），密码只回报 `hasPassword`，明文永远不出服务端。`config.yaml` 以 0600 权限原子写入。
