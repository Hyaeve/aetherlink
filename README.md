...
| 页面 | 作用 |
| --- | --- |
| 以太链接 | 每个上游一张卡片，卡片直接显示名称、服务端类型、`地址:端口`、`上游端口 → 反代端口`与状态（缺密钥 / 已停用 / 端口未监听）。左键卡片打开详细编辑窗口，右键弹出菜单（详细编辑 / 测试连接 / 停用启用 / 删除）。保存立即生效。 |
| 运行日志 | 上半是**播放流水**：每条播放请求最终是 302、透传、中继还是失败，逐条列出请求路径、跳转目标、耗时，可按结果过滤；一次都没 302 时页面会直接给出最可能的原因。下半是内存环形缓冲的运行日志，按级别过滤。 |
| 系统设置 | 302 跳转策略、缓存、日志级别、修改管理账号。 |
