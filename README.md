...
登录窗口的账号会自动填好（后端自举时一并给出当前账号名，改过名也能对上），只剩密码要输。密码框**默认就是明文**——这里的框打开时是空的，遮蔽只影响打字时看不看得见，所以默认让你看清自己输的是什么；要遮起来点输入框右侧的小眼睛（斜线眼睛 = 当前可见，点一下变圆点）。上游编辑窗口里的密钥 / 密码相反，默认是圆点。
...
- 管理界面有独立账号，与上游账号无关。首次启动写入内置的 admin / password，请尽快在设置页改掉。未登录访问管理 API 一律返回 401；只有 `/aetherlink/api/health` 与 `/aetherlink/api/bootstrap` 免鉴权。bootstrap 只回版本号、密码长度下限与**当前账号名**——账号名要用来预填登录页的账号框，本身不是秘密；凭据、密码校验材料，以及「是否仍在用默认账号」这些一律不回，扫端口的人拿不到「这里 admin/password 就能进」这种结论。
