...
| `中继上游断流` / `中继写回客户端失败` / `中继不完整` | 中继明细行里的「转发 x（长度 y）」说明 AetherLink 已经把上游声明的字节转发给了客户端。`中继上游断流` 是上游（含网盘驱动）提前断流，播放必然卡住或失败——问题在源，不在 AetherLink；`中继写回客户端失败` 是客户端先走了（播放器断开、网络中断），日志会写明已转发的字节数，用来和上一种区分开；`中继不完整` 是转发量低于上游声明的长度。中继正常收尾也会打一行明细，写明上游状态码、转发字节、客户端 `Range`、`Accept-Ranges` 与**回给客户端的 `Content-Type`**。`中继被客户端中断` 也会记一行并写明已转发字节：拖动进度条、切集的正常形态与「播放器不认我们回的响应」共用这个出口，靠字节数区分。 |
| `中继 ...（状态 206，几百 ms）` 之后播放器立刻报错，且日志里没有别的明细 | 字节已经从中继取回来了，是播放器读完响应头就断开——「`中继被客户端中断`」那行的「已转发 x」会写明它到底吃了多少：只有几十 KB、远小于 `Range` 要的长度，就是播放器主动离开，先查回给它的 `Content-Type`（网盘 CDN 一律回 `application/octet-stream`，类型由 AetherLink 按直链扩展名补，见「已知取值边界」）；已接近 `Range` 长度则只是拖了进度条。 |
...
- 中继模式转发 `Range`、`If-Range`、`Content-Range`，seek 行为与直连一致。回给客户端的 `Content-Type` 以上游为准，只在它是空或 `application/octet-stream` 时按直链扩展名补一个：`.mp4` / `.m4v` → `video/mp4`，`.m4a` / `.m4b` → `audio/mp4`，`.mkv` → `video/x-matroska`。网盘 CDN（115 等）对视频正是回 `application/octet-stream`，所以这条兜底在中继里几乎总在生效，而且**必须给出视频类型**：客户端请求的 `/stream.{容器}` 与直链真实容器不一致时，`Content-Type` 是它唯一的格式线索，标成音频会让它读完响应头就断开。
