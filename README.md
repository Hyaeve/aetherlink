...
| 项 | 说明 |
| --- | --- |
| 跳转模式 | 在上游卡片中设置：`always` 始终跳转；`public` 公网客户端 302、内网客户端中继；`private` 内网客户端 302、公网客户端中继；`never` 始终中继。按播放客户端 IP 判断，不按访问域名或媒体直链域名判断。无法识别 IP 时，条件跳转模式回退中继。例外：开启「先跟随上游 302」后，若跳转链走完仍是原地出流的内网地址（OpenList「本地代理」形态）而客户端在外网，则改由 AetherLink 中继——302 出去客户端也连不上，日志会写明原因。 |
