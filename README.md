...
- **拿到账号**：使用上游项目自带的 API 密钥（Audiobookshelf 的 Key 就是一枚 JWT，可作 Bearer 使用；Emby 与飞牛影视用 `X-Emby-Token`/`api_key`），因此 AetherLink 能以该账号权限查询书库、条目和文件真实路径。
...
2. 进入「以太链接」，点虚线的「添加上游」卡片：选服务端类型（Audiobookshelf、Emby 或飞牛影视），填上游地址、反代端口和对应的 API 密钥——Audiobookshelf 填「设置 → 用户 → 该用户的 API Token」，Emby 填「控制台 → 高级 → API 密钥」，飞牛影视填它「设置 → 高级」里的 API 密钥。保存前可以点「试连」当场确认密钥是否可用。
...
| 能播但日志里全是 `passthrough`，没有 302 | 上游把这一轨报告成了普通文件。Emby / 飞牛影视侧检查项目是不是真的 `.strm` 库（AetherLink 认 `Protocol: Http` 或 `Container: strm`）；Audiobookshelf 侧确认媒体目录已挂载。飞牛影视还要确认密钥可用——它的接口都在 `/emby` 前缀下，密钥不对时 API 只会返回 SPA 的 HTML，于是永远解析不到媒体源（客户端的播放协商请求若漏了 `/emby` 前缀，AetherLink 会自动补上，这条不必自己改播放器配置）。界面「运行日志」页的播放流水里，这一行的「目标」列会写明具体原因。 |
...
| 服务端类型 | 选 Audiobookshelf 就填 Audiobookshelf 的 API Token（设置 → 用户 → 该用户的 API Token）；选 Emby 就填 Emby 的 API 密钥（控制台 → 高级 → API 密钥）；选飞牛影视就填飞牛影视「设置 → 高级」里的 API 密钥。飞牛影视是 Emby 方言，走的是同一套拦截与 302 链路。 |
