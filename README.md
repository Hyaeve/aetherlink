...
| `中继 ...（状态 206，几百 ms）` 之后播放器立刻报错，且日志里没有别的明细 | 字节已经从中继取回来了，是播放器读完响应头就断开——「`中继被客户端中断`」那行的「已转发 x」会写明它到底吃了多少：只有几十 KB、远小于 `Range` 要的长度，就是播放器主动离开，先查回给它的 `Content-Type`（网盘 CDN 与移动云 EOS 一律回 `application/octet-stream`，类型由 AetherLink 按响应体开头的魔数补，认不出来才退回直链扩展名，见「已知取值边界」）；已接近 `Range` 长度则只是拖了进度条。若同一行前后还出现「`播放器可能据此选错解复用器`」，就是库里的名字与真实文件对不上（客户端按上游的 `Container` 请求 `/stream.mkv`，而直链的字节是 MP4）：这条直链走 302 时播放器拿到中性类型、自己按内容嗅探所以正常，改成中继才会暴露——按「已知取值边界」里那条 `Content-Type` 规则给类型只是兜底，根治要修库里的 STRM 名或上游的 `Container`。 |
...
- 中继模式转发 `Range`、`If-Range`、`Content-Range`，seek 行为与直连一致。回给客户端的整幅响应头以上游为准（`Content-Length`、`Content-Range`、`Accept-Ranges`、`ETag` 都原样带过去，保持定长响应、绝不退化成 chunked，否则播放器拿不到总长度、moov 在文件尾部的 MP4 无法 seek）。`Content-Type` 只在它是空或 `application/octet-stream` 时自己补，顺序是**先字节、后扩展名**：先看**响应体开头的魔数**（`ftyp` 家族按品牌分视频与纯音频、EBML 的 Matroska 与 WebM、RIFF 的 AVI 与 WAV、`OggS`、`fLaC`、ASF；只对从文件开头开始的单段 `Range` 生效，用于判断的字节原样写回客户端，不丢一字节），魔数认不出来才退回直链扩展名（`.mp4` / `.m4v` → `video/mp4`，`.mkv` → `video/x-matroska`，`.ts` → `video/mp2t` 等）。网盘 CDN（115 等）与移动云 EOS 对视频都回 `application/octet-stream`，直链路径又常常只有一串 ID、根本没有扩展名可猜，所以字节这一层是必需的：线上就有过一条直链，路径是 `/50f351580c084c969c0c84f4e500ed39086`、`Content-Disposition` 里的文件名写着 `.iso`，字节其实是 `ftypisom…mp41`（MP4）。**反过来也不能让扩展名压过字节**：网盘上「文件名写 `.mkv`、内容其实是 MP4」很常见，那种直链按扩展名回 `video/x-matroska` 就是让播放器拿 matroska 去解 MP4 字节，读完几百 KB 就断开——而同一条直链走 302 时播放器拿到的是中性类型、自己按内容嗅探，反倒正常，于是表现成「302 能播、中继播不了」。唯一的例外是纯音频扩展名（`.m4a` / `.m4b` / `.m4p` / `.mp3` / `.flac` / `.opus` / `.ogg` / `.aac` / `.wav` / `.wma` / `.mka` / `.webma`）：MP4 家族里有声书与视频共用容器头，品牌字段常是 `isom` / `mp42`，字节根本分不出「这是音频」，扩展名才是唯一线索，所以这一类直接采信扩展名、不再看字节。客户端请求的 `/stream.{容器}` 与直链真实容器不一致时，`Content-Type` 是它唯一的格式线索，标成音频或留成中性类型都可能让它读完响应头就断开。
