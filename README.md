...
**所有配置都在网页上完成。** compose 只需要一个 `./config` 卷：管理账号、上游地址与 API 密钥、302 策略、路径映射全部在管理界面填写，保存即生效、无需重启容器。
...
1. 用默认账号 **admin / password** 登录，登录框已经预填好，直接点登录即可。
...
4. 播放端把 Audiobookshelf 地址换成 `http://宿主机:15151` 即可。

> 端口暴露在局域网时请务必先改掉默认账号：任何能访问这个端口的人都能用 admin / password 登录并读到上游 API 密钥。界面顶部与设置页会一直提示，直到你改掉为止。
...
- 管理界面有独立账号，与上游账号无关。首次启动写入内置的 admin / password，请尽快在设置页改掉。未登录访问管理 API 一律返回 401；只有 `/aetherlink/api/health` 与登录页自举信息免鉴权，后者不含任何上游地址、密钥或路径。
