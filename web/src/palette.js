...
export function gradientFor(name, startedAt = '') {
  const [from, via, to] = GRADIENTS[hash(`${startedAt}:${name}`) % GRADIENTS.length]
  return `linear-gradient(145deg, ${from} 0%, ${via} 45%, ${to} 100%)`
}

// 负延迟让动画一进页面就停在随机相位，不会出现所有卡片同时从头开始。
export function tideDelayFor(name, startedAt = '') {
  return `-${hash(`${startedAt}:${name}:phase`) % TIDE_SECONDS}s`
}

// 卡片要的全部内联样式：渐变 + 动画相位。
export function cardStyleFor(name, startedAt = '') {
  return { backgroundImage: gradientFor(name, startedAt), animationDelay: tideDelayFor(name, startedAt) }
}
