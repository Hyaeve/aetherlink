...
- **音频适配缓存的空目录由收尾那一方删掉**：iOS 音频适配把 AAC 重新封装 / WMA 转码的结果放在 `/cache/<书名>/<键>/<原文件名>.m4a`，两层目录都是 AetherLink 自己建的，所以一本书的这些文件全删掉后，这两层空目录也要跟着消失——留着只会让 `/cache` 越攒越乱，还让人以为缓存仍在。判断「空了」不先数一遍文件：`os.Remove` 对非空目录本来就失败，`pruneEmptyDirectories` 从键目录往上删到第一次失败为止，既省一次遍历，也顺带保证目录里还有文件（正在读写的缓存）时不会被删走；缓存根目录通常是 compose 挂进来的挂载点，不碰。配套一处顺序要求：`getOrCreate` 的 `MkdirAll` 必须排在 `cleanupLocked` **之后**——那次清理会把这本过期变体所在的空目录链一并删掉，目录要是提前建好就会被它删走，紧接着的 `CreateTemp` 直接报 `ENOENT`（`TestAudioCacheRebuildsDirectoryRemovedByCleanup` 守着这一条）。
