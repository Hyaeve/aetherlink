...
- **移动云盘直链封顶 15 分钟**：交给客户端的直链（`FinalURL`，未跟随跳转时即 `Target.URL`）里出现 `cmecloud.cn` 子串时，缓存有效期不超过 `cmecloudCacheTTL`。按子串而不是主机名后缀匹配 —— 这个词可能落在路径或查询参数里，而直链形态由上游给的地址决定，我们控制不了。判定只看这两条 URL、不看 `Hops`：中间跳是我们预检时自己走掉的，客户端不会去请求它。**这条规则只往下压不往上抬**：直链自带 `t` 时那个时间才是它真正的寿命，`t` 比 15 分钟短就按 `t`，`t` 已过期（`ttl` 为 0，等于不入缓存）也必须保持不入缓存 —— 否则等于把一条死链当成新链存下来。固定规则，没有配置项。
- **直链缓存跨重启保留**：解析结果原子写入 `direct-links-cache.json`，仅恢复未过期条目；键包含媒体引用、UA、播放器身份摘要和媒体配置摘要。配置摘要覆盖服务器地址、类型、凭据、路径规则等，改变这些值后不会命中旧服务器的缓存；只改跳转模式、启停或监听端口不使缓存失效。旧版本无隔离字段的缓存不会命中，待到期自然淘汰。写入缓存后再结束并发任务，避免完成与入缓存之间出现重复解析窗口。
