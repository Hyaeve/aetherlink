...
| `internal/auth` | 管理账号（用户名 + 口令）的 PBKDF2-SHA256 派生与恒定时间校验、内置默认账号的生成，以及会话令牌存储（只存令牌哈希；普通会话只在内存，勾了「保持登录」的会连同过期时间落到 `sessions.json`）。 |
...
- **会话分两类，「保持登录」才落盘**：令牌本身在两种情况下都不写配置文件，内存里存的也只是它的 SHA-256（内存被 dump 也换不出可用令牌）。不勾「保持登录」时是普通会话，12 小时有效、随用顺延，容器重启即失效；勾了则签发「保持登录」会话，**固定 7 天、不随使用顺延**，并把指纹与过期时间原子写进 `/config/sessions.json`（0600），启动时只恢复其中未过期的那些。刻意不做滑动续期：随用随顺延会让「维持 7 天」变成「只要还在用就永不过期」，等于把一份本来会自动失效的凭据变成永久凭据。注销单个会话、`RevokeAll()`（改密码）都会同步重写这个文件——改密码的意思就是「谁都别再进来」，留一条能免密进来的记录与它相悖。文件读坏或解析失败只记一行日志、当作没有会话，不让服务起不来。
