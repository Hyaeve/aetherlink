...
2. Emby 系（`emby` / `fnos`）的 `/Items/:id/PlaybackInfo` 先原样请求上游。STRM 源只有在 `SupportsDirectPlay=true` 时才重写 `DirectStreamUrl`，三个 `Supports*` 能力字段与 `TranscodingUrl` 全部保留；若上游判断客户端不兼容，则响应逐字节不改并继续 HLS 转码。例外：该上游的跳转模式为 always 或 never（卡片「始终跳转」/「始终中继」）时，STRM 源压成 `SupportsDirectPlay=false` + `SupportsDirectStream=true` 并补 `DirectStreamUrl`，同时把 `TranscodeReasons` / `TranscodingReasons` 清成空数组。压成 DirectStream 是无条件的：既堵掉上游判可直放时的 DirectPlay——那种情况下客户端会直连 `Path`（网盘 / OpenList 直链）绕开 AetherLink，既没有播放记录，也不受内网直链安全网保护——也是这两档「流量全经 AetherLink」的前提；只有「上游说这个客户端不能直放」的那一半还受卡片里「无视上游的不可直放判定」开关控制（默认开＝照样引回；关掉＝这类客户端交回上游转码，只认上游 HLS 的播放器需要它，日志会在保留转码那行的原因后注明是开关关着）。这两档都要压，是因为它们都已指定流量去向（302 或中继），客户端不回 AetherLink 就等于设置失效；`public` / `private` 两档不同，见下方「Emby 的 302 起点是 PlaybackInfo」那条。普通媒体源始终不修改。飞牛影视同一个响应里还会顺手给 `MediaStreams` 补上非空字段，并在转发前把这条请求钉到 `/emby` 前缀下（`upstream.RequestPathRewriter`）——少了前缀飞牛返回的是单页应用 HTML，拿不到 `MediaSources` 就永远不 302。
...
- **Emby 的 302 起点是 PlaybackInfo，不是 HLS 分片**：客户端先根据 `PlaybackInfo` 决定直放或转码。一旦选中 `/hls1/main/*.ts`，每个请求只代表一段转码数据，不可能 302 到完整媒体文件。但也不能把所有 STRM 强制直放：网页端拿到不支持的 H.265 原文件同样无法播放。因此只在上游给出 `SupportsDirectPlay=true` 时重写 `DirectStreamUrl`，其余能力字段和转码 URL 保持原样；不兼容时宁可不 302，也要让 Emby 正常转码。媒体源和兼容性判断会一起短暂缓存；不兼容的客户端即使又请求 `/stream`，也会退回上游而不会误跳原文件。例外是卡片「始终跳转」与「始终中继」这两档：它们都已经显式指定了流量去向（302 出去，或由 AetherLink 自己中继），都要求客户端必须先回到 AetherLink 的 `/stream`，所以 STRM 源不再按上游判定分流，一律压成 DirectStream（`DirectPlay` 绝不能用——客户端会拿它直连 `Path` 绕开我们）。其中「上游说不能直放」的那一半另受卡片开关「无视上游的不可直放判定」控制（默认开）：关掉之后这类客户端回到上游转码，这在全是「能直放的客户端」的假设下看着像退化，但确实有播放器只认上游 HLS——实测 AfuseKt 拿到原始文件直链后读几十 KB（正好读完 MKV 的 `Tracks`）就断开，这类客户端被强行引回只会从「能播」变成「播不了」。判定被推翻之后，紧接着那条 `/stream` 请求才会真的去解析直链；若还沿用「听上游的」行为，它会读到 `ErrDirectPlayUnsupported` 就地透传，中继链路压根不启动、日志里只剩一行「透传」。反过来，「公网跳转」「内网跳转」只在 302 与中继之间按客户端来源二选一，因此保留上游判定——客户端真的解不了原文件时，上游转码是唯一的出路。这条区别由 `claimsSources`（模式要不要接管流量）与 `ignoresVerdict`（要不要连判定一起推翻）两张表定，四种模式与开关的三种取值都在 `fnos_test.go` 里正反成对钉住。
