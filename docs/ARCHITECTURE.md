...
2. Emby 系（`emby` / `fnos`）的 `/Items/:id/PlaybackInfo` 先原样请求上游。STRM 源只有在 `SupportsDirectPlay=true` 时才重写 `DirectStreamUrl`，三个 `Supports*` 能力字段与 `TranscodingUrl` 全部保留；若上游判断客户端不兼容，则响应逐字节不改并继续 HLS 转码。例外：该上游的跳转模式为 always 或 never（卡片「始终跳转」/「始终中继」）时，STRM 源无条件压成 `SupportsDirectPlay=false` + `SupportsDirectStream=true` 并补 `DirectStreamUrl`，同时把 `TranscodeReasons` / `TranscodingReasons` 清成空数组。无条件是必须的：既忽略上游的不可直放判定（飞牛对外网客户端的码率限制判定会让转码流量全部走上游自己），也堵掉上游判可直放时的 DirectPlay——那种情况下客户端会直连 `Path`（网盘 / OpenList 直链）绕开 AetherLink，既没有播放记录，也不受内网直链安全网保护。这两档都要压，是因为它们都已指定流量去向（302 或中继），客户端不回 AetherLink 就等于设置失效；`public` / `private` 两档不同，见下方「Emby 的 302 起点是 PlaybackInfo」那条。普通媒体源始终不修改。飞牛影视同一个响应里还会顺手给 `MediaStreams` 补上非空字段，并在转发前把这条请求钉到 `/emby` 前缀下（`upstream.RequestPathRewriter`）——少了前缀飞牛返回的是单页应用 HTML，拿不到 `MediaSources` 就永远不 302。
